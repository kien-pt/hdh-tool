...
function LinkedIndex() {
  const [values, setValues] = React.useState({
    ...
    fileSize: 20, fileSizeUnit: 1024 * 1024,
    position: 12, positionUnit: 1024 * 1024,
  });
  ...
  const handleSubmit = (event) => {
    ...
    const totalPointersPerBlock = parseInt(blockSize / pointerSize) - 1;

    const blockId = parseInt(postion / blockSize);
    const blockIndex = parseInt(blockId / totalPointersPerBlock);
    const blockOffset = blockId % totalPointersPerBlock;
    const offset = postion % blockSize;
  ...
  const res = [
    { width: 4, key: 'blockIndex', label: 'Block Index', value: values?.blockIndex },
    { width: 4, key: 'blockOffset', label: 'Block Offset', value: values?.blockOffset },
    { width: 4, key: 'offset', label: 'Offset', value: values?.offset },
  ];
